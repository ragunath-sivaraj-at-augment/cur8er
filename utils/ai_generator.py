...
class NanoBananaProFeatures:
    ...
    def _build_enhanced_prompt(base_prompt, use_search_grounding=False, text_rendering_mode=False):
        ...
        if use_search_grounding:
            prompt_parts.append(
                "Use current real-world knowledge and verified information. "
                "Ensure factual accuracy for any maps, diagrams, or informational content."
            )
        
        # Advanced text rendering
        if text_rendering_mode:
            prompt_parts.append(
                "Generate clear, professional text within the image. "
                "Use appropriate typography with proper font selection and natural text placement. "
                "Ensure text is readable and well-integrated into the design."
            )
        
        # Nano Banana Pro quality enhancements
        prompt_parts.extend([
    ...
    def detect_advanced_features(prompt):
        ...
        search_keywords = [
            'infographic', 'facts', 'map', 'diagram', 'chart', 'data', 
            'real', 'accurate', 'current', 'geography', 'statistics'
        ]
        
        text_keywords = [
            'text', 'typography', 'sign', 'poster', 'banner', 'label', 
            'headline', 'slogan', 'tagline', 'words', 'font'
        ]
        
        use_search = any(keyword in prompt.lower() for keyword in search_keywords)
        use_text = any(keyword in prompt.lower() for keyword in text_keywords)
